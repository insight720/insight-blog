...
public class AsyncScheduleProperties {
    ...
    /**
     * 要保留在池中的核心线程数，即使它们处于空闲状态。（仅用于 TaskExecutor）
     */
    private Integer corePoolSize;

    /**
     * 保留在池中的核心线程数，即使它们处于空闲状态。（仅用于 TaskScheduler）
     */
    private Integer schedulePoolSize;

    /**
     * 当队列中存放的任务数量达到队列容量后，池中的最大线程数量。（仅用于 TaskExecutor）
     */
    private Integer maxPoolSize;

    /**
     * 线程数大于核心线程数时，多余的空闲线程在终止之前等待新任务的最长秒数。（仅用于 TaskExecutor）
     */
    private Integer keepAliveSeconds;

    /**
     * 池中运行的线程数量达到核心线程数后，任务队列的容量。（仅用于 TaskExecutor）
     */
